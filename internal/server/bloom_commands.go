...
func (s *Server) executeBloom(args [][]byte) ([]byte, error) {
	...
	case "BF.RESERVE":
		...
		if err := s.store.BloomReserve(key, errorRate, capacity); err != nil {
			return nil, err
		}
		return []byte("+OK\r\n"), nil
	...
	case "BF.INSERT":
		...
		results, err := s.store.BloomInsert(key, capacity, errorRate, args[itemsAt:])
		if err != nil {
			return nil, err
		}
		return bloomBoolArray(results), nil
