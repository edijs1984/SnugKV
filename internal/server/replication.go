...
// appendBacklogPayloadLocked takes ownership of payload. Callers must not mutate
// the slice after passing it here; replica queues may concurrently reference the
...
func encodePlainSetReplicationFrame(key, value []byte) []byte {
	...
	headLen := len(payload) - frameLen - 2
	return append([]byte(nil), payload[headLen:headLen+frameLen]...)
}

func encodePlainSetReplicationPayload(key, value []byte) (int, []byte) {
	// Build the RESP bulk envelope and compact SET frame in one allocation.
	// The returned payload is immutable and can be shared by the backlog and
	// replica queues.
	const headerLen = 12
	const checksumLen = 4
