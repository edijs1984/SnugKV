...
func TestJSONType(t *testing.T) {
	...
	if got := execute(t, s, "JSON.TYPE", "user", "$"); got != "*1\r\n$6\r\nobject\r\n" {
		t.Fatalf("JSON.TYPE root got %q", got)
	}

	if got := execute(t, s, "JSON.TYPE", "user", "$.name"); got != "*1\r\n$6\r\nstring\r\n" {
		t.Fatalf("JSON.TYPE string got %q", got)
	}

	if got := execute(t, s, "JSON.TYPE", "user", "$.age"); got != "*1\r\n$7\r\ninteger\r\n" {
...
func TestJSONDel(t *testing.T) {
	...
	if got := execute(t, s, "JSON.GET", "user", "$.profile.city"); got != "$2\r\n[]\r\n" {
