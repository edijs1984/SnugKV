...
func TestEncodeGeneralSkipsLikelyIncompressibleValue(t *testing.T) {
	r := NewRegistry()
	value := make([]byte, 256)
	x := uint64(1)
	for i := range value {
	...
	if !likelyIncompressible(value) {
		t.Fatal("random value was not classified as likely incompressible")
	}

	rec := r.EncodeGeneral(value, false)
	if rec.ID != Raw {
		t.Fatalf("codec = %s, want raw", r.Name(rec.ID))
	}
	if !bytes.Equal(rec.Data, value) {
		t.Fatal("raw fallback changed bytes")
	}
}

func TestEncodeGeneralStillCompressesRepetitiveValue(t *testing.T) {
	r := NewRegistry()
	value := bytes.Repeat([]byte("snugkv-benchmark-"), 32)

	if likelyIncompressible(value) {
		t.Fatal("repetitive value was classified as incompressible")
	}

	rec := r.EncodeGeneral(value, false)
	if rec.ID != LZ4 {
		t.Fatalf("codec = %s, want lz4", r.Name(rec.ID))
	}
}

func TestIncompressibilityGateKeepsRepeatedHighDiversityPattern(t *testing.T) {
	block := make([]byte, 64)
	for i := range block {
		block[i] = byte(i)
	}
	value := bytes.Repeat(block, 4)

	if likelyIncompressible(value) {
		t.Fatal("repeated high-diversity pattern was classified as incompressible")
	}
