...
func TestJSONPathTopLevelProjections(t *testing.T) {
	root, err := Parse([]byte(`{"a":2,"b":4,"arr":[1,2,3]}`))
	if err != nil {
		t.Fatal(err)
	}
	...
	cases := []struct {
		...
		{"$.a * $.b", []any{float64(8)}},
		{"($.a + $.b) / 2", []any{float64(3)}},
		{"$.arr.length()", []any{float64(3)}},
		{"length($.arr)", []any{float64(3)}},
		{"-$.a", []any{float64(-2)}},
		{"$.a / 0", []any{}},
	}
...
func TestJSONPathParenthesizedPlainPathIsNotProjection(t *testing.T) {
