...
func main() {
	keys := flag.Int("keys", 100000, "number of sorted-set keys")
	members := flag.Int("members", 8, "members per sorted set")
	memberBytes := flag.Int("member-bytes", 16, "bytes per sorted-set member")
	...
	if *keys <= 0 || *members <= 0 || *memberBytes < 10 {
		fatalf("keys and members must be positive; member-bytes must be at least 10")
	}

	items := dataset(*members, *memberBytes)
	...
	loadStart := time.Now()
	for i := 0; i < *keys; i++ {
		if _, _, _, err := store.ZSetAdd(zsetKey(i), items, engine.ZSetAddOptions{}); err != nil {
			fatalf("SnugKV ZADD %q: %v", zsetKey(i), err)
		}
	}
	...
	sample, ok, err := store.ZSetStorageStats(zsetKey(0))
	if err != nil || !ok {
		fatalf("read SnugKV sample sorted-set stats: found=%t err=%v", ok, err)
	}
	...
	redisStart := time.Now()
	if err := redis.LoadZSets(*keys, items); err != nil {
		fatalf("Redis ZADD load: %v", err)
	}
	...
	fmt.Printf("ZSET benchmark\n")
	fmt.Printf("keys: %d\n", *keys)
	fmt.Printf("members_per_zset: %d\n", *members)
	fmt.Printf("member_bytes: %d\n", *memberBytes)
	fmt.Printf("member_bytes_total_per_zset: %d\n", sample.MemberBytes)
	fmt.Printf("snug_packed_bytes_per_zset: %d\n", sample.PackedBytes)
	fmt.Printf("snug_stored_bytes_per_zset: %d\n", sample.StoredBytes)
	...
	fmt.Printf("bytes_per_zset_delta: %.2f\n", perZSet(snugDelta, *keys))
	fmt.Printf("index_delta: %d\n", indexDelta)
	fmt.Printf("index_bytes_per_zset: %.2f\n", perZSet(indexDelta, *keys))
	fmt.Printf("entry_delta: %d\n", entryDelta)
	fmt.Printf("entry_bytes_per_zset: %.2f\n", perZSet(entryDelta, *keys))
	fmt.Printf("arena_delta: %d\n", arenaDelta)
	fmt.Printf("arena_bytes_per_zset: %.2f\n", perZSet(arenaDelta, *keys))
	fmt.Printf("arena_payload_delta: %d\n", payloadDelta)
	fmt.Printf("arena_payload_bytes_per_zset: %.2f\n", perZSet(payloadDelta, *keys))
	fmt.Printf("arena_live_block_delta: %d\n", liveBlockDelta)
	fmt.Printf("arena_live_block_bytes_per_zset: %.2f\n", perZSet(liveBlockDelta, *keys))
	fmt.Printf("meta_delta: %d\n", metaDelta)
	fmt.Printf("meta_bytes_per_zset: %.2f\n", perZSet(metaDelta, *keys))
	...
	fmt.Printf("entry_storage_bytes_per_zset: %.2f\n", perZSet(layoutAfter.EntryStorageBytes-layoutBefore.EntryStorageBytes, *keys))
...
func dataset(count, memberBytes int) []engine.ZSetItem {
	out := make([]engine.ZSetItem, count)
	for i := 0; i < count; i++ {
		...
		copy(member, prefix)
		out[i] = engine.ZSetItem{Score: float64(i), Member: member}
	}
	return out
}

func zsetKey(i int) string { return fmt.Sprintf("zset:%08d", i) }
func perZSet(n uint64, keys int) float64 { return float64(n) / float64(keys) }
func fatalf(format string, args ...any) {
	fmt.Fprintf(os.Stderr, "zsetbench: "+format+"\n", args...)
	os.Exit(1)
}
...
func (c *redisClient) LoadZSets(keys int, items []engine.ZSetItem) error {
	const batchSize = 512
	for start := 0; start < keys; start += batchSize {
		...
		for i := start; i < end; i++ {
			args := make([][]byte, 0, 2+len(items)*2)
			args = append(args, []byte("ZADD"), []byte(zsetKey(i)))
			for _, item := range items {
				args = append(args, []byte(strconv.FormatFloat(item.Score, 'g', -1, 64)), item.Member)
			}
