...
- Search: JSON-backed `FT.CREATE`, `FT.SEARCH`, `FT.AGGREGATE`, `FT.INFO`, `FT.DROPINDEX`, and `FT._LIST` with TAG/NUMERIC/TEXT/GEO fields, exact/grouped/unqualified TEXT terms, prefixes/suffix/contains wildcards, exact phrases, stemming/NOSTEM, stopwords, LANGUAGE/LANGUAGE_FIELD, SLOP/INORDER, fuzzy queries, `WEIGHT`/`SORTABLE`/`NOINDEX`, `PHONETIC dm:en`, BM25STD-style relevance ranking with `WITHSCORES`, Redis-audited GEO radius filters over `"lon,lat"` JSON strings, and an aggregate pipeline with `LOAD`, `FILTER`, `GROUPBY`, reducers, `SORTBY`, and `LIMIT`.
