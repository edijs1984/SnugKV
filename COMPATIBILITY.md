...
For queries without `SORTBY`, SnugKV does not promise Redis's incidental result ordering. JSON object field serialization order may also differ while preserving the same document content. Schema modifiers `WEIGHT`, `SORTABLE`, and `NOINDEX` are supported for the audited JSON Search field forms. `NOINDEX` suppresses postings; `SORTABLE` is retained as metadata, while `SORTBY` remains usable on non-SORTABLE fields as in Redis. Unqualified TEXT queries search across all indexed TEXT fields. Redis-audited suffix and contains TEXT wildcards are also supported (`*ory`, `*mor*`) for fielded and unqualified queries, including grouped forms and measured escaping/error behavior. Audited support includes multiple implicit-AND terms, phrases, prefix and fuzzy forms, stopwords, stemming/NOSTEM, mixed fielded clauses, and DIALECT 1/2 parity for the measured cases. TEXT fields support the Redis-audited `PHONETIC dm:en` subset for exact term lookup, including unqualified queries and measured modifier/error behavior. The current Search implementation is intentionally narrower than the complete Redis Search feature set.
