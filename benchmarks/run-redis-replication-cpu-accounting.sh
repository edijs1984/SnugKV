...
BENCH_BIN="${BENCH_BIN:-/tmp/rediswirebench-redis-cpuprof}"
...
wait_dbsize() {
  ...
  while (( SECONDS < deadline )); do
    ...
    if [[ "$size" == "$expected" ]]; then echo "$size"; return 0; fi
    sleep 0.05
  done
...
start_redis() {
  ...
  local port="$2"
  shift 2
  "$REDIS_SERVER"     --bind 127.0.0.1     --protected-mode no     --port "$port"     --save ""     --appendonly no     --loglevel warning     --logfile ""     "$@"     >"/tmp/redis-cpuprof-${port}.log" 2>&1 &
  local pid=$!
  PIDS+=("$pid")
  NAMES+=("$name")
  wait_ping "$port"
...
for p in 6410 6411 6412 6413 6414; do
  old="$(lsof -ti tcp:$p 2>/dev/null || true)"
  [[ -n "$old" ]] && kill "$old" 2>/dev/null || true
done
sleep 0.5

start_redis primary "$PRIMARY_PORT"
for i in 0 1 2 3; do
  port=$((BASE_REPLICA_PORT + i))
  start_redis "replica$((i+1))" "$port" --replicaof 127.0.0.1 "$PRIMARY_PORT"
...
start_ns="$(date +%s%N)"
/usr/bin/time -f 'client_user_sec=%U\nclient_sys_sec=%S\nclient_cpu_pct=%P'   -o /tmp/redis-cpu-profile-client.time   "$BENCH_BIN"   -server redis-cpu-profile-4rep   -addr "127.0.0.1:${PRIMARY_PORT}"   -workload load   -keys "$KEYS"   -workers "$WORKERS"   -pipeline "$PIPELINE"   -value-bytes "$VALUE_BYTES"   -value-shape "$VALUE_SHAPE"   -reset   > /tmp/redis-cpu-profile-bench.json
end_ns="$(date +%s%N)"
...
echo
echo "========== BENCHMARK =========="
cat /tmp/redis-cpu-profile-bench.json
...
echo
echo "========== CLIENT CPU =========="
cat /tmp/redis-cpu-profile-client.time
